...
import type { FlightAvailability, PageBomFlight } from '../types.ts'
...
export const FlightResult = ({ flight, flightKey, savedFlights, staticFilesPath }: FlightResultProps): JSX.Element => {
    ...
    if (numF > 0)
        available.push(
            <span class="bulk_cabin bulk_f">
                F <b>{numF}</b>
            </span>
        )
    if (numJ > 0)
        available.push(
            <span class="bulk_cabin bulk_j">
                J <b>{numJ}</b>
            </span>
        )
    if (numPY > 0)
        available.push(
            <span class="bulk_cabin bulk_p">
                PY <b>{numPY}</b>
            </span>
        )
    if (numY > 0)
        available.push(
            <span class="bulk_cabin bulk_y">
                Y <b>{numY}</b>
            </span>
        )
    ...
    if (flight.segments.length === 1) {
        return (
            <div class="flight_wrapper">
                <div
                    class={classNames('flight_item', 'direct', {
                        saved: savedFlights.has(flightKey),
                        f: numF > 0,
                        j: numJ > 0,
                        py: numPY > 0,
                        y: numY > 0,
                    })}
                    data-flight-key={flightKey}
                    data-flight-avail={`${numF}_${numJ}_${numPY}_${numY}`}
                >
                    <img
                        src={`https://book.cathaypacific.com${staticFilesPath}common/skin/img/airlines/logo-${leg1Airline.toLowerCase()}.png`}
                    />
                    <span class="flight_num">
                        {leg1Airline}
                        {leg1FlightNum}
                    </span>
                    {...available}
                    ...
                    <span class="flight_save">
                        <Heart className="heart_save" />
                    </span>
                </div>
                <div class="flight_info">
                    <span class="info_flight">
                        {leg1Airline}
                        {leg1FlightNum} ({leg1Origin.slice(-3)} ✈ {leg1Dest.slice(-3)})
                    </span>
                    <span class="info_dept">
                        <span>Departs:</span> {leg1DepTime}
                    </span>
                    <span class="info_arr">
                        <span>Arrives:</span> {leg1ArrTime}
                    </span>
                    <span class="info_duration">
                        <span>Total Flight Duration:</span> {duration}
                    </span>
                </div>
            </div>
        )
    } else {
        const transitTime = formatFlightDuration(
            flight.segments[1].flightIdentifier.originDate - flight.segments[0].destinationDate
        )
        ...
        return (
            <div class="flight_wrapper">
                ...
                <div class="flight_info">
                    ...
                    <span class="info_transit">
                        <span>Transit Time:</span> {transitTime}
                    </span>
                    <span class="info_flight">
                        {leg2Airline}
                        {leg2FlightNum} ({leg2Origin.slice(-3)} ✈ {leg2Dest.slice(-3)})
                    </span>
                    <span class="info_dept">
                        <span>Departs:</span> {leg2DepTime}
                    </span>
                    <span class="info_arr">
                        <span>Arrives:</span> {leg2ArrTime}
                    </span>
                    <span class="info_duration">
                        <span>Total Flight Duration:</span> {duration}
                    </span>
